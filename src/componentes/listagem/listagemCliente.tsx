...
function ListagemCliente() {
  ...
  const carregarClientes = () => {
    try {
      const dadosTitular = localStorage.getItem('clientesTitulares')
      if (dadosTitular) {
        setTitulares(JSON.parse(dadosTitular))
      }

      const dadosDependente = localStorage.getItem('clientesDependentes')
      if (dadosDependente) {
        setDependentes(JSON.parse(dadosDependente))
      }
    } catch (error) {
  ...
  const encontrarNomeTitular = (idTitular: number) => {
    const titular = titulares.find(t => t.id === idTitular)
